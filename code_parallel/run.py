...
def main():
    ...
    ap.add_argument("--endgame-f64", action="store_true",
                    help="run the LBFGS/f64 endgame in float64 (slow on GeForce; "
                         "default is fast float32-on-GPU, ~1e-7 floor).")
    ...
    ap.add_argument("--polish", action="store_true",
                    help="run the full expensive float64 endgame on the --fast "
                         "consensus (push_precision recipe: re-query teacher in "
                         "float64, staged MSE->MAE LBFGS, ~1e-7 floor) instead of "
                         "the fast fp32 solve (~1e-3/1e-4). Only affects --fast.")
    ...
    if args.fast and os.path.exists(fast_dump):
        ...
        if cons is None:
            ...
            cons = cons.to(device)
            if args.polish:
                # full float64 endgame on the same consensus/queries (--fast
                # early stop preserved). Re-queries the teacher in float64
                # (query-only) and grinds the flat directions down to the
                # identifiability floor. Measured in float64 for a true e-7.
                teacher64 = teacher.double().to(device)
            ...
            final = {
                ...
                "final_agree": agreement(cons, meas_teacher, meas_pts),
