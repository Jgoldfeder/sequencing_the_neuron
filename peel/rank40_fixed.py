...
sgn=torch.where(Cm[torch.arange(80),ci]<Cp[torch.arange(80),ci],-1.0,1.0); W2c_al=W2c[ci]*sgn[:,None]
A=(W2c_al@B.t()).clone()
...
def Lobj(A):   # TRUE certificate: bottom-40 energy ratio (recompute SVD)
...
def lsqr(Aop,Atop,b,n,damp,iters=80):
    ...
    return x
print(f"start werr {werr(A)*100:.4f}%  bot40 {Lobj(A):.3e}",flush=True)
lam=1e-4; traj=[]; nrej=0
for it in range(801):
    e=werr(A)*100; L0=Lobj(A); traj.append(e)
    if it in (0,25,50,100,200,400,600,800):
        print(f"  it{it:4d}: werr {e:.5f}%  bot40 {L0:.3e}  lam {lam:.0e}  rej_so_far {nrej}",flush=True); torch.save({"traj":traj,"A":A.cpu()},"rank40_fixed_traj.pt")
    with torch.no_grad():
    ...
    for _ in range(9):
        ...
        if Lobj(An) < L0:          # ACCEPT iff TRUE certificate decreases (recomputed SVD)
            A=An; lam=max(lam/2,1e-12); ok=True; break
        lam*=4
    if not ok: nrej+=1; lam=min(lam,1e2)
