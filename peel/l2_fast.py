"""FAST L2 diagnostic (float32, single init, lighter Jacobian). Answers in one run:
 (1) CHEAT positive control  : true correspondence + true downstream -> does W2 converge? (validates harness)
 (2) HONEST W2-only          : guessed downstream (member-0 frame), refine W2 only
 (3) HONEST joint            : guessed downstream frame, refine W2 + b2 + downstream
Same optimizer/config for all three. Prints wall-clock per phase.
"""
import sys, time, torch
from torch.func import jvp, vjp
torch.set_default_dtype(torch.float32)          # <-- 3090 has ~no fp64; target is 1e-4 so fp32 is plenty
...
# committee -> two consensus seeds: truth-aligned (for cheat) and member-0-frame (honest)
mem=[{k:v.to(dev).float() for k,v in sd.items()} for sd in pk["pop_states"]]
def consensus(ref):   # align all members' L2 rows to ref, cluster, mean; ref is (Wref) 80x128
    A=[]
    for sd in mem:
    ...
    for j in range(80):
        cl=((A[:,j]-A[:,j].median(0).values).norm(dim=1)/A[:,j].median(0).values.norm().clamp_min(1e-9))<0.2
        if int(cl.sum())>=5: med[j]=A[cl,j].mean(0)
    return med
seed_true=consensus(W2t)                              # truth-aligned consensus (cheat frame)
seed_mem0=consensus(mem[0]["layers.1.weight"])        # member-0-frame consensus (honest)
...
def refine(w0,resid,iters=40):
    ...
    for it in range(iters):
        ...
        for _ in range(10):
            ...
            if cn<c:wf=wn;r=rn;c=cn;lam=max(lam*0.3,1e-12);ok=True;break
            lam*=5
...
def resid_W2(model):
    ...
    return r
TRUE=(bt[1],Wt[2],bt[2],Wt[3],bt[3],Wt[4],bt[4])
GUESS=(gb2,gW3,gb3,gW4,gb4,gW5,gb5)

def timed(tag, seed, resid):
    t=time.time(); m0=wscore(seed); wf,c=refine(seed.reshape(-1).clone(),resid); m1=wscore(wf.reshape(80,128))
    print(f"  {tag:38s} {m0[0]:.3e}/{m0[1]:.2e} -> {m1[0]:.3e}/{m1[1]:.2e}  <1%:{m1[2]}/80  loss {c:.2e}  [{time.time()-t:.1f}s]")

print(f"float32, N={N}, m={M}.  W rel-row mean/max.")
...
# (3) honest joint
seedJ=[seed_mem0,gb2,gW3,gb3,gW4,gb4,gW5,gb5]
shapes=[t.shape for t in seedJ];numels=[t.numel() for t in seedJ]
def unpack(v):
    o=0;out=[]
    for sh,n in zip(shapes,numels): out.append(v[o:o+n].reshape(sh));o+=n
    return out
def resid_J(v):
    ps=unpack(v);W2=ps[0];b2,W3,b3,W4,b4,W5,b5=ps[1:]
    fh=lambda hb:(lambda q2,q3,q4:q4@W5.t()+b5)(torch.sigmoid(hb@W2.t()+b2),0,0) if False else None
    def mm(hb):
        q2=torch.sigmoid(hb@W2.t()+b2);q3=torch.sigmoid(q2@W3.t()+b3);q4=torch.sigmoid(q3@W4.t()+b4);return q4@W5.t()+b5
    val=mm(h);dd=torch.func.vmap(lambda u:jvp(mm,(h,),(u.expand(N,128),))[1])(Q)
    return torch.cat([((val-valt)/scv).reshape(-1),((dd-ddt)/scd).reshape(-1)])
t=time.time();v0=torch.cat([x.reshape(-1) for x in seedJ]).clone();m0=wscore(seed_mem0)
wf,c=refine(v0,resid_J);m1=wscore(unpack(wf)[0])
print(f"  {'(3) HONEST guessed-down, joint':38s} {m0[0]:.3e}/{m0[1]:.2e} -> {m1[0]:.3e}/{m1[1]:.2e}  <1%:{m1[2]}/80  loss {c:.2e}  [{time.time()-t:.1f}s]")
