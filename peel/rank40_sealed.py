...
NB=200; gh2=torch.Generator(device=dev).manual_seed(7); H=torch.sigmoid(torch.randn(NB,128,generator=gh2,device=dev)*1.0).clamp(2e-2,1-2e-2)
...
lam=1e-6
for it in range(301):
    if it in (0,25,50,100,200,300): print(f"  it{it:3d}: werr {werr(A@Brec)*100:.5f}%  bot40 {botr(A):.3e}",flush=True)
    with torch.no_grad():
