...
# ACTUAL consensus W2 (member-0 gauge) -> align to teacher gauge -> project to true rowspace B
...
sgn=torch.where(Cm[torch.arange(80),ci]<Cp[torch.arange(80),ci],-1.0,1.0)
W2c_al=W2c[ci]*sgn[:,None]                     # aligned to teacher ordering/sign
A=(W2c_al@B.t()).clone()
...
def lsqr(Aop,Atop,b,n,damp,iters=80):
    ...
    return x
print(f"consensus start werr {werr(A)*100:.4f}% (projected to true B, aligned)",flush=True)
lam=1e-6; traj=[]
for it in range(1201):
    e=werr(A)*100; traj.append(e)
    if it in (0,25,50,100,200,400,600,800,1000,1200):
        print(f"  it{it:4d}: werr {e:.5f}%  bot40 {botr(A):.3e}",flush=True); torch.save({"traj":traj,"A":A.cpu()},"rank40_cons_traj.pt")
    with torch.no_grad():
