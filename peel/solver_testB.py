"""TEST B (reviewer): fix the q->h_true coordinate warp. Candidate models the layer-2 input as
hc=sigmoid(logit(q)+db1) with db1 a 128-dim L1-bias warp NUISANCE (init 0), whose Jacobian is exactly
the J_h the earlier candidate ignored. Project out (eta, db1) tangent; step A only. One step per member.
If coherent members now step truth-ward, the coordinate mismatch (confound #2) was the cause. Sealed.
"""
...
def match(A,Bm):
    ...
    return torch.tensor(r,device=dev),torch.tensor(cc,device=dev)
...
B=recover_B(); print(f"[testB] B recovered; q={NQ[0]}",flush=True)
...
print(f"[testB] targets built; q={NQ[0]}",flush=True)
logitH=torch.log(H/(1-H)); Hqf=H*(1-H)
...
def observ(A,eta,db1):
    b2,W3,b3,W4,b4,W5,b5=ueta(eta)
    zeta=logitH+db1; hc=torch.sigmoid(zeta); g=(hc*(1-hc))/Hqf; W2=A@B
    z2=hc@W2.t()+b2; s2=torch.sigmoid(z2); sp2=s2*(1-s2)
    z3=s2@W3.t()+b3; s3=torch.sigmoid(z3); sp3=s3*(1-s3)
    z4=s3@W4.t()+b4; s4=torch.sigmoid(z4); sp4=s4*(1-s4)
    out=s4@W5.t()+b5
    dhc=g[:,None,:]*B[None,:,:]
    dz2=dhc@W2.t(); ds2=sp2[:,None,:]*dz2; dz3=ds2@W3.t(); ds3=sp3[:,None,:]*dz3
...
def lsqr(Aop,Atop,b,n,damp,iters=60):
    ...
    return x
def proj_step(A0,em):
    A=A0.clone(); db0=torch.zeros(128,device=dev)
    r=(observ(A,em,db0)-target)/vsc
    nuis=torch.cat([em,db0])
    Jn=jacrev(lambda n:(observ(A,n[:4962],n[4962:])-target)/vsc,chunk_size=256)(nuis); Q,_=torch.linalg.qr(Jn)
    fA=lambda a:(observ(a.reshape(80,80),em,db0)-target)/vsc
    ...
    for _ in range(10):
        d=lsqr(Aop,Atop,-rp,6400,damp); An=A+d.reshape(80,80); rn=(observ(An,em,db0)-target)/vsc; rpn=rn-Q@(Q.t()@rn)
...
for m in range(8):
    ...
    A0=W2m@B.t(); A1=proj_step(A0,em); results[m]=((A0@B).cpu(),(A1@B).cpu())
    print(f"[testB] member {m} done",flush=True)
torch.save({"results":results},PEEL+"testB_out.pt")
print(f"[testB] total q={NQ[0]}",flush=True); _send({"cmd":"stop"}); c.close()
