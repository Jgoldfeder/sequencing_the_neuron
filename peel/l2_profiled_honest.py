"""HONEST profiled landscape: optimize eta from the 8 COMMITTEE downstreams (multistart), take min --
NO true-eta oracle. Compare to the true-eta-assisted value. If honest-min tracks the oracle value and
stays monotone toward truth, VarPro can work honestly; if honest-min is stuck high / non-monotone, the
earlier monotone landscape was an oracle artifact. true B, oracle jets (value+1st)."""
import sys, torch
...
def aligned_eta(sd):
    ...
    return torch.cat([b2a.reshape(-1),W3a.reshape(-1),b3a,sd["layers.3.weight"].reshape(-1),sd["layers.3.bias"],sd["layers.4.weight"].reshape(-1),sd["layers.4.bias"]])
etas=[aligned_eta(sd) for sd in mem]                       # 8 committee downstreams (true order)
...
target=observ(W2t,eta_true).detach(); vsc=float(target.abs().max()); sig=float(target.pow(2).mean())**0.5
...
def opt(W2,e0,iters=30):
    ...
    return c
def rms(c): return (c*(vsc**2)/target.numel())**0.5/sig
print("[honest profiled] min over 8 committee-downstream inits vs true-eta init. value+1st jets.")
print(f"{'t':>4} {'W2err':>7} {'honest min(8)':>14} {'true-eta init':>14}")
for tt in [0.0,0.3,0.6,1.0]:
    W2=(1-tt)*W2guess+tt*W2t; we=float(((W2-W2t).norm(dim=1)/nt).mean())
    hs=min(opt(W2,e) for e in etas); te=opt(W2,eta_true)
    print(f"{tt:>4.1f} {we:>7.3f} {rms(hs):>14.3e} {rms(te):>14.3e}",flush=True)
