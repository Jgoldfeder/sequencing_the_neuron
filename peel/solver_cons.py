...
def _match(Ai,Bi):
    Cp=torch.cdist(Ai,Bi);Cm=torch.cdist(-Ai,Bi);C=torch.minimum(Cp,Cm)
    import scipy.optimize as so; r,cc=so.linear_sum_assignment(C.cpu().numpy())
...
_ref=mem[0]["layers.1.weight"]; _AW=[_ref.clone()]
for _sd in mem[1:]:
    _Wm=_sd["layers.1.weight"]; _r,_c,_s=_match(_Wm,_ref); _Wa=torch.zeros_like(_Wm); _Wa[_c]=_Wm[_r]*_s[:,None]; _AW.append(_Wa)
W2cons=torch.stack(_AW).median(0).values           # truth-free consensus (~16%), member-0 gauge
Rm=W2cons-(W2cons@B.t())@B; A=W2cons@B.t()
...
for it in range(50):
    ...
    torch.save({"traj":traj,"lams":lams},PEEL+"cons_out.pt")
    print(f"[iter {it:2d}] lambda={lam} q={NQ[0]}",flush=True)
torch.save({"traj":traj,"lams":lams},PEEL+"cons_out.pt"); print(f"[iter] done q={NQ[0]}",flush=True); _send({"cmd":"stop"}); c.close()
