...
sig=lambda z:1/(1+np.exp(-z)); logit=lambda p:np.log(p/(1-p)); print('built')
def run(seed, Pn=5):
    ...
    st1=sig(av*tps[0]+bt); bg=bt+0.05*(2*rng.random()-1); sg=sig(av*tps[0]+bg)   # bias GUESS -> s1 guess
    spg=np.array([sig(logit(sg)+av*(tps[p]-tps[0])) for p in range(Pn)])
    def resid(v):
    ...
    for i in range(120):
        ...
        x0=np.concatenate([r.uniform(-3,3,2), spg+0.01*(2*r.random(Pn)-1), r.uniform(0.05,0.95,Pn), r.uniform(0.05,0.95,Pn)])
        lo=np.concatenate([[-20,-20],np.zeros(3*Pn)]); hi=np.concatenate([[20,20],np.ones(3*Pn)])
        x0=np.clip(x0,lo+1e-4,hi-1e-4)
        try: sol=least_squares(resid,x0,method='trf',bounds=(lo,hi),max_nfev=600,xtol=1e-15,ftol=1e-15)
        except Exception: continue
        if sol.cost<1e-18:
            sv=sol.x[2:2+Pn]; Pv=sol.x[2+Pn:2+2*Pn]; Qv=sol.x[2+2*Pn:2+3*Pn]
            if np.all(sv>1e-3)&np.all(sv<1-1e-3)&np.all(Pv>0)&np.all(Pv<1)&np.all(Qv>0)&np.all(Qv<1): roots.append(sol.x[2])
    u=[]
    for v in sorted(roots):
        if not u or abs(v-u[-1])>1e-6: u.append(v)
    if not u: return None
    spick=min(u,key=lambda v:abs(v-sg)); b_rec=logit(spick)-av*tps[0]
    return len(u), int(any(abs(v-st1)<1e-5 for v in u)), abs(b_rec-bt)
print('COUPLED P=5 + trf+bounds + guess-init, 8 nets:')
errs=[]
for sd_ in range(8):
    res=run(sd_)
    if res: errs.append(res[2]); print(f'  seed {sd_}: #roots={res[0]}  true-found={res[1]}  bias err={res[2]:.2e}')
    else: print(f'  seed {sd_}: none found')
errs=np.array(errs)
if len(errs): print(f'SUMMARY: bias err median={np.median(errs):.2e} max={errs.max():.2e}  frac<1e-4={(errs<1e-4).mean():.0%} frac<1e-6={(errs<1e-6).mean():.0%}')
