...
W1r=W1t; b1r=b1t; W1rp=W1r.t()@torch.linalg.inv(W1r@W1r.t())
...
print(f"[TRUE-L1 DIAG] start werr {werr(A@Brec)*100:.4f}%  bot40 {botr(A,b2):.3e}",flush=True)
for it in range(5001):
    A=p[:6400].reshape(80,80); b2=p[6400:]
    if it%500==0: print(f"  it{it:4d}: werr {werr(A@Brec)*100:.5f}%  bot40 {botr(A,b2):.3e}",flush=True); torch.save({"p":p.cpu(),"Brec":Brec.cpu()},"rank40_trueL1_traj.pt")
    with torch.no_grad():
