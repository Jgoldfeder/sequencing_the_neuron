...
if __name__ == "__main__":
	...
	# reuse a cached black-box if one with identical arch/dataset/epochs/activation/seed exists
	bb_key = utils.blackbox_cache_key(args.model_type, args.layers, args.activation, args.dataset, args.num_epochs, args.seed, seqlens=[28])
	utils.load_or_train_blackbox(model, bb_key, num_epochs=args.num_epochs, dataset=args.dataset, model_type=args.model_type, seqlens=[28])
	# start reconstruction timer (excludes black-box training above)
	...
	# Scale batch size with number of GPUs (samples_per_gpu samples per GPU)
	...
	with torch.enable_grad():
		for outer_iter in range(args.outer_iterations):
			...
			# Copy models to each GPU once per outer iteration (models change after train_one_epoch)
			gpu_model_copies = None
			if gpu_ids is not None and len(gpu_ids) > 1:
				import copy
				gpu_model_copies = {gpu_id: [copy.deepcopy(s).cuda(gpu_id) for s in subslist] for gpu_id in gpu_ids}
			...
			for i in range(10):
				population.train_one_epoch(batch_size=args.batch_size, epoch_num=i, restore=False)
				sys.stdout.flush()
	...
	# Reconstruction wall-clock time (excludes black-box training)
	recon_time = time.time() - recon_start
	print(f"Reconstruction wall-clock time (excl. black-box training): {recon_time:.2f}s")
	print(f"Reconstruction wall-clock time (excl. black-box training): {recon_time:.2f}s", file=sys.__stdout__)
